...
def load_stock_data(selected_ticker):
    if selected_ticker not in tickers:
        return f"잘못된 티커를 입력하셨습니다.", None
    
    # yfinance로 주식 데이터 불러오기
    df = yf.download(selected_ticker, start=start_date, end=end_date)
    
    if df.empty:
        return f"{tickers[selected_ticker]}에 대한 데이터가 없습니다.", None
    
    # 열 이름 매핑 (Adj Close -> Adj_Close)
...
def analyze_stock(selected_ticker):
    ...
    if df.empty:
        return None, None, None, f"{tickers[selected_ticker]}에 대한 데이터가 없습니다."
    ...
    def plot_graph(data_df, future_dates, future_mean_predictions, future_std_predictions, title):
        ...
        plt.plot(data_df['Date'], data_df['Close'], label=f'{selected_ticker} Actual Close Prices', color='#A1C6EA')
